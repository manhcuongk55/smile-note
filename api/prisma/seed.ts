...
async function main() {
    ...
    // 3. Buildings spread across Ho Chi Minh City
    ...
    const getRoomFor = (buildingId: string, floor: number) =>
        allRooms.find(r => r.buildingId === buildingId && r.floor === floor);

    // 5. Tasks — diverse types and priorities, spread across buildings
    const taskData = [
        // URGENT — scattered across city
    ...
    // 6. Contracts — various stages
    const vacantRooms = allRooms.filter(r => r.status === 'VACANT');

    // Contract 1: ACTIVE — đang thuê
    ...
    // Contract 2: DEPOSITED — đã đặt cọc, chờ ký
    await prisma.contract.create({
        data: {
            status: 'DEPOSITED',
            monthlyRent: 6500000,
    ...
    // Contract 3: CONSULTING — đang tư vấn
    await prisma.contract.create({
        data: {
            status: 'CONSULTING',
            monthlyRent: 7000000,
