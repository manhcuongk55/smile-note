...
export class RouteService {
  ...
  private priorityWeight(priority: string): number {
    switch (priority) {
      case 'URGENT':
        return 0.3; // reduce effective distance by 70%
      case 'HIGH':
        return 0.6;
      case 'MEDIUM':
        return 1.0;
      case 'LOW':
        return 1.5; // push further
      default:
        return 1.0;
    }
  }
  ...
  async getOptimizedRoute(managerId?: string) {
    ...
    const buildingMap = new Map<
      string,
      { building: TaskWithBuilding['building']; tasks: TaskWithBuilding[] }
    >();
    ...
    const buildingEntries = Array.from(buildingMap.entries()).map(
      ([id, data]) => {
        const maxPriority = data.tasks.reduce(
          (max, t) => {
            const rank =
              priorityRank[t.priority as keyof typeof priorityRank] || 0;
    ...
    while (visited.size < buildingEntries.length) {
      ...
      for (const entry of buildingEntries) {
        ...
        const dist = this.haversineKm(
          current.building.latitude,
          current.building.longitude!,
          entry.building.latitude,
          entry.building.longitude!,
        );
    ...
    const stops: RouteStop[] = route.map((entry, index) => {
      ...
      if (index > 0) {
        ...
        if (prev.building.latitude && entry.building.latitude) {
          distFromPrev = this.haversineKm(
            prev.building.latitude,
            prev.building.longitude!,
            entry.building.latitude,
            entry.building.longitude!,
          );
        }
      ...
      const taskMinutes = sortedTasks.reduce(
        (sum, t) => sum + (t.estimatedDuration || 20),
        0,
      );
